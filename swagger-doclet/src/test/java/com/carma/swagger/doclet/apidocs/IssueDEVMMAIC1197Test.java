...
public class IssueDEVMMAIC1197Test {
    ...
    public void setUp() {
        ...
        this.docletOptions = docletOptions.setRecorder(this.recorderMock).setIncludeSwaggerUi(false);
    }

    @Test
    public void testStart() throws IOException {
        final RootDoc rootDoc = RootDocLoader.fromPath("src/test/resources", "fixtures.issueDEVMMAIC1197");
        Assert.assertNotNull(rootDoc);
        boolean result = new JaxRsAnnotationParser(this.docletOptions, rootDoc).run();
        Assert.assertTrue(result);
        final ApiDeclaration apiDeclaration = loadFixture("/fixtures/issueDEVMMAIC1197/fruits.json", ApiDeclaration.class);
